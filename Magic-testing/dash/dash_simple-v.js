...
face[0] = {
	...
	old:ew.def.bpp?0:1,
	init: function(o){ 
		//this.spdC=process.env.BOARD=="BANGLEJS2"?[0,0,13,13]:[15,14,15,14];
		this.ampC=[1,0,13,13];
	...
	spdf: function(){
		...
		this.g.setColor(1,euc.dash.alrt.spd.cc==2?13:0);
		...
		this.g.drawString(Math.round(this.spd*this.spdF),this.gui.spdm+10-(this.g.stringWidth(Math.round(this.spd*this.spdF))/2),this.gui.spdm-(this.gui.txt/2)); 
	...
	barBat: function(){
		...
		let rel=(this.gui.bat[3]-this.gui.bat[1])/100;

		this.g.setColor(1,1);
		this.g.fillRect({x:this.gui.bat[0],y:this.gui.bat[1],x2:this.gui.bat[2],y2:this.gui.bat[3]-(this.bat*rel),r:10});

		this.g.setColor(0,11);
		this.g.fillRect({x:this.gui.bat[0],y:this.gui.bat[3]-(this.bat*rel),x2:this.gui.bat[2],y2:this.gui.bat[3],r:10});	

		this.g.setFontVector(20);
		this.g.setColor(0,2);
		this.g.drawString("|    |    |    |",120-(this.g.stringWidth("|    |    |    |")/2),this.gui.bat[1]);
