TC.removeAllListeners("tc5");
face[0].page="app";
face[0].btn={};
UI.ele.ind("top",2,2);
let img;
if (require("Storage").read("calc",2)) {
	img=E.toArrayBuffer(atob(""));
	UI.btn.img("_2x3",1,img,"Calc",15,2);
}else UI.ele.fill("_2x3",1,2);
//app2
if (require("Storage").read("repellent",1)) {
	img=E.toArrayBuffer(atob(""));
	UI.btn.img("_2x3",2,img,"Repel",15,2);
}else UI.ele.fill("_2x3",2,2);
//app3
if (global.tpms) {
	img=require("heatshrink").decompress(atob("mEwwIWT/ACBh+AApnwgPAg4FCgfgAoMBAofAgPgAoMDAowBC8AJBDIIFBEIIFFh8AAoV4hwFDnAFFjgFDjgFLnAFDhwFlgB7BAoYABAoxBBAARHCAoqbBAoMeFIc8NYp9EnEHAodwV4QdBWII/BAAKpCAgKnBAIIFBXIX/AAQFGgAFDEIP8+fnz7vC+PHx4FHAAg"));
	UI.btn.img("_2x3",3,img,"TPMS",15,2);
}else UI.ele.fill("_2x3",3,2);
//app4
if (require("Storage").read("hello",1)) {
	img=require("heatshrink").decompress(atob(""));
	UI.btn.img("_2x3",4,img,"Hello",15,2);
}else  UI.ele.fill("_2x3",4,2);
//app5
UI.ele.fill("_2x3",5,2);//img=E.toArrayBuffer(atob(""));
//app6
UI.ele.fill("_2x3",6,2);
img=0;
UI.btn.img("_bar",1,UI.icon.torch,0,14,0);//btn2
UI.btn.img("_bar",2,UI.icon.settings,0,14,0);//btn2
UI.btn.img("_bar",3,UI.icon.alarm,0,14,0);//btn2
if (set.def.bpp) w.gfx.flip();
//
face[0].btn._2x3_1=()=>{buzzer(buz.ok);face.go('calc',0);};
face[0].btn._2x3_2=()=>{buzzer(buz.ok);face.go('repellent',0);};
face[0].btn._2x3_3=()=>{buzzer(buz.ok);face.go('tpmsFace',0);};
face[0].btn._2x3_4=()=>{buzzer(buz.ok);face.go('hello',0);};
face[0].btn._2x3_5=()=>{buzzer(buz.na);};
face[0].btn._2x3_6=()=>{buzzer(buz.na);};
//bar
face[0].btn._bar_1=()=>{buzzer(buz.na);};
face[0].btn._bar_2=()=>{
	if (Boolean(require("Storage").read(face.faceSave[0].substring(0,2)+"Options"))){
		buzzer(buz.ok);face.go(face.faceSave[0].substring(0,2)+"Options",0);
	}else buzzer(buz.na);
};
face[0].btn._bar_3=()=>{buzzer(buz.ok);face.go('alarm',0);};