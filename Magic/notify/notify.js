...
face[0] = {
  ...
  init: function(){
    this.g.setColor(0);
	this.g.fillRect(0,0,239,239);this.g.setColor(1);this.g.fillRect(0,0,170,77);this.g.fillRect(175,0,239,77);this.g.fillRect(0,83,170,157);this.g.fillRect(175,83,239,157);this.g.fillRect(0,163,170,249);this.g.fillRect(175,163,239,249);
	this.g.flip();
	this.g.setColor(14);	
  ...
  show : function(){
	...
	if (this.nCall!=notify.nCall){
		...
		this.g.setColor((this.nCall)?7:1);
	...
	if (this.nInfo!=notify.nInfo){
		...
		this.g.setColor((this.nInfo)?12:1);
...
face[5] = {
  ...
  show : function(){
    ...
    if (this.del===1) {
      ...
      this.g.setFont("Vector",24);
	  if (this.type==="call") this.g.setColor(7);
	  else if (this.type==="im") this.g.setColor(4);
	  else if (this.type==="info") this.g.setColor(12);
      this.g.fillRect(0,0,239,30);
      //if (this.msg.idUnread) {this.g.setColor(15);set.gbSend({t:"notify", id:this.msg.id, n:"dismiss"});} else this.g.setColor(2);
...
touchHandler[0]=function(e,x,y){
    if (e==5){
	  ...
	  if (y>160&&x<50) {
		...
		if (face.appPrev=="main")buzzer(ew.pin.BUZZ,0,40);
		else face.go("main",0);return;
    }else if  (e==12){
      if (y<80&&x<170){
		buzzer(ew.pin.BUZZ,0,[30,50,80]);notify.call=[];notify.nCall=0;face[0].nCall=-1;
		if (!notify.nCall&&!notify.nIm&&!notify.nInfo) {set.gbSend({t:"notify", n:"dismiss_all"});notify.New=0;}
      } else if (80<=y&&y<160&x<170){
		buzzer(ew.pin.BUZZ,0,[30,50,80]);notify.im=[];notify.nIm=0;face[0].nIm=-1;
		if (!notify.nCall&&!notify.nIm&&!notify.nInfo) {set.gbSend({t:"notify", n:"dismiss_all"});notify.New=0;}
	  } else if (160<=y&&y<239&x<170){
		buzzer(ew.pin.BUZZ,0,[30,50,80]);notify.info=[];notify.nInfo=0;face[0].nInfo=-1;
		if (!notify.nCall&&!notify.nIm&&!notify.nInfo) {set.gbSend({t:"notify", n:"dismiss_all"});notify.New=0;}
      }else buzzer(ew.pin.BUZZ,0,40);
    }
   face.off();
    ...
    if (face[5].del===2){
      if  (e==3) {
        ...
        if (notify[face[5].type].length==0){
			buzzer(ew.pin.BUZZ,0,80); notify["n"+face[5].type.substr(0,1).toUpperCase()+face[5].type.substr(1)]=0; 
