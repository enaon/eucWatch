...
face[0] = {
	...
	init: function(){
		...
		this.btn(1,"ALARM 3",18,60,115,7,6,0,100,119,195,"80 %",28,60,150);
			...
			if (x<=120&&y<100) { //alarm 1
                if (euc.dash.spd1E){
					...
					if (euc.dash.spd2E){
						...
						buzzer(ew.pin.BUZZ,0,[30,50,30]);
                    }else {face[0].ntfy("ENABLE ALARM 2","",20,7,1);buzzer(ew.pin.BUZZ,0,40);}
                }
			}else if (120<=x<=239&&y<=100) { //alarm 2
				if ( euc.dash.spd2E){
					if (euc.dash.spd1E) {face[0].ntfy("DISABLE ALARM 1","",20,7,1);buzzer(ew.pin.BUZZ,0,40);}
					else {
						euc.dash.spd2E=0;
						buzzer(ew.pin.BUZZ,0,[30,50,30]);
