		...
		if (face[0].set) { 
			...
			if (face[0].set=="spd") {
				...
				if (y<=120){ //spd
			        ...
			        face[0].btn(1,"RESOLUTION:",18,120,110,4,0,0,100,239,195,euc.dash.spdS,50,120,140);
					if (!spdr) { spdr=1;spds=0;
						face[0].btn(1,"SPEED (in "+((set.def.dash.mph)?"MPH)":"Km/h)"),18,120,8,1,0,0,0,239,97,(set.def.dash.mph)?(euc.dash[euc.dash.haSv]*0.625).toFixed(1):euc.dash[euc.dash.haSv],50,120,40);
						face[0].ntfy("1 PULSE PER "+euc.dash.spdS+((set.def.dash.mph)?" MPH":"KPH")+" > "+((set.def.dash.mph)?Math.round(euc.dash[euc.dash.haSv]*0.625):euc.dash[euc.dash.haSv]),"",18,4,1);
					}
				}  
				...
				if (y<=65){ //uphill
                    ...
                    face[0].btn(1,"UPHILL:",20,60,23,4,0,0,0,239,63,euc.dash.ampH+" A",35,180,16);
			        face[0].btn(1,"BRAKING:",20,60,90,1,0,0,66,239,132,euc.dash.ampL+ " A",35,182,84);
			        face[0].btn(1,"RESOLUTION:",17,70,157,1,0,0,135,239,195,euc.dash.ampS+ " A",35,190,150);
			        ...
			        face[0].btn(1,"BRAKING:",20,60,90,4,0,0,66,239,132,euc.dash.ampL+ " A",35,182,84);
                    face[0].btn(1,"UPHILL:",20,60,23,1,0,0,0,239,63,euc.dash.ampH+" A",35,180,16);
			        face[0].btn(1,"RESOLUTION:",17,70,157,1,0,0,135,239,195,euc.dash.ampS+ " A",35,190,150);
					face[0].ntfy("ALERT IF UNDER "+euc.dash.ampL+" A","",18,1,1);                    
				}else {//RESOLUTION
					if (120<=x&&euc.dash.ampS<3) euc.dash.ampS++;
					else if (x<=120&&1<euc.dash.ampS) euc.dash.ampS--;
			        face[0].btn(1,"RESOLUTION:",17,70,157,4,0,0,135,239,195,euc.dash.ampS+ " A",35,190,150);
			        face[0].btn(1,"BRAKING:",20,60,90,1,0,0,66,239,132,euc.dash.ampL+ " A",35,182,84);
                    face[0].btn(1,"UPHILL:",20,60,23,1,0,0,0,239,63,euc.dash.ampH+" A",35,180,16);
              ...
              if (y<=120){ //
					...
					face[0].btn(1,"SET HI-TEMP (in "+((set.def.dash.farn)?"F)":"C)"),18,120,8,4,0,0,0,239,97,(set.def.dash.farn)?(euc.dash.tmpH*1.8+32).toFixed(1):euc.dash.tmpH,50,120,40);
					face[0].ntfy("ALERT IF OVER "+((set.def.dash.farn)?Math.round(euc.dash.tmpH*1.8+32):euc.dash.tmpH)+((set.def.dash.farn)?" F":" C"),"",18,4,1);
				}else{ //back
		            w.gfx.setColor(0);
              ...
              if (y<=120){ //
                    ...
                    face[0].btn(1,"SET LOW-BATT (in %)",18,120,8,4,0,0,0,239,97,euc.dash.batL,50,120,41);
					face[0].ntfy("ALERT IF UNDER "+euc.dash.batL+" %","",18,4,1);
				}else{ //back
           		    w.gfx.setColor(0);
					w.gfx.drawLine (120,0,120,97);
					w.gfx.drawLine (121,0,121,97);
			...
			//if (set.read("dash","slot"+set.read("dash","slot")+"Maker")!="Kingsong") 
				face[0].btn(1,"SPEED (in //"+((set.def.dash.mph)?"MPH)":"Km/h)"),18,120,8,1,0,0,0,239,97,(set.def.dash.mph)?(euc.dash[euc.dash.haSv]*0.625).toFixed(1):euc.dash[euc.dash.haSv],50,120,40);
			//else 
			//	face[0].btn(1,"SPEED (in "+((set.def.dash.mph)?"MPH)":"Km/h)"),18,120,8,1,0,0,0,239,97,(set.def.dash.mph)?(euc.dash.lim[euc.dash.haSv]*0.625).toFixed(1):euc.dash.lim[euc.dash.haSv],50,120,40);
			face[0].btn(1,"RESOLUTION:",18,120,110,2,0,0,100,239,195,euc.dash.spdS,50,120,140);
			...
			face[0].set="temp";
            buzzer(ew.pin.BUZZ,0,[30,50,30]);
            face[0].btn(1,"SET HI-TEMP (in "+((set.def.dash.farn)?"F)":"C)"),18,120,8,12,0,0,0,239,97,(set.def.dash.farn)?(euc.dash.tmpH*1.8+32).toFixed(1):euc.dash.tmpH,50,120,40);
		}else if (120<=x&&100<=y) { //batt
			face[0].set="batt";
            buzzer(ew.pin.BUZZ,0,[30,50,30]);
            face[0].btn(1,"SET LOW-BATT (in %)",18,120,8,12,0,0,0,239,97,euc.dash.batL,50,120,40);
