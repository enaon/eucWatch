...
face[0] = {
	...
	show : function(){
		...
		if (notify.ring){
			...
			this.ring=0;
			var colbt=1;
			if (this.bt==3)  colbt=6;
			else if (this.bt==4)  colbt=8;
			else if (this.bt==2)  colbt=9;
			this.g.setColor(colbt);
			this.g.fillRect(0,0,158,50); //date
			this.g.setColor(14);
	...
	widg:function(){
		...
		if (notify.ring){
			if (this.ring!=notify.ring){
				this.ring=notify.ring;this.g.setColor(0);this.g.clearRect(0,151,239,239);this.g.setColor(15);
			...
			if (notify.nCall||notify.nIm||notify.nInfo){
				...
				else if (this.nInfo)  {this.msg=JSON.parse(notify.info[0]);this.cf=6;}
				this.g.setColor(15);//
				this.g.setFont("Vector",25);
	...
	time:function(){
		...
		if (this.t[1]!=this.min ){
			...
			if (global.alrm) {
				...
				else if (alrm[1].tmr!==-1||alrm[2].tmr!==-1||alrm[3].tmr!==-1) {this.bmin=4;this.fsec=15;this.bsec=4;}
				else  {this.bmin=1;this.fsec=15;this.bsec=1;}
			}else {this.bmin=1;this.fsec=15;this.bsec=1;}
			this.g.setColor(this.bmin);
			this.g.fillRect(100,55,203,150);
			this.g.setColor(this.fmin);
			this.g.drawString(this.t[1],107,69);
			//this.g.flip();
		}
		//seconds
		this.g.setColor(this.bsec);
		this.g.fillRect(203,55,240,150);
		this.g.setColor(this.fsec);//
...
touchHandler[0]=function(e,x,y){
	...
	//alarms
	   ...
	   if (alrm.buzz!=-1) {
		alrm.stop(alrm.now); buzzer(ew.pin.BUZZ,0,[80,40,80]);
	   }else {
        buzzer(ew.pin.BUZZ,0,[30,50,30]);
