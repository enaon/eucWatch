...
touchHandler[0]=function(e,x,y){ 
	...
	case 5: //tap event
		if (face[0].set) { 
			...
			if (face[0].set=="spd") {
				...
				if (y<=120){ //spd
					...
					if (!spds) { spds=1;spdr=0;
						face[0].btn(1,"RESOLUTION:",18,120,110,1365,0,0,100,239,195,euc.dash.spdS,50,120,140);
					...
					if (!spdr) { spdr=1;spds=0;
						face[0].btn(1,"SPEED (in "+((set.def.dash.mph)?"MPH)":"Km/h)"),18,120,8,1365,0,0,0,239,97,(set.def.dash.mph)?(euc.dash[euc.dash.haSv]*0.625).toFixed(1):euc.dash[euc.dash.haSv],50,120,40);
				...
				if (y<=65){ //uphill
			        ...
			        face[0].btn(1,"BRAKING:",20,60,90,1365,0,0,66,239,132,euc.dash.ampL+ " A",35,182,84);
			        face[0].btn(1,"RESOLUTION:",17,70,157,1365,0,0,135,239,195,euc.dash.ampS+ " A",35,190,150);
					face[0].ntfy("ALERT IF OVER "+euc.dash.ampH+" A","",18,1365,1);
                    ...
                    face[0].btn(1,"UPHILL:",20,60,23,1365,0,0,0,239,63,euc.dash.ampH+" A",35,180,16);
			        face[0].btn(1,"RESOLUTION:",17,70,157,1365,0,0,135,239,195,euc.dash.ampS+ " A",35,190,150);
					face[0].ntfy("ALERT IF UNDER "+euc.dash.ampL+" A","",18,1365,1);                    
				}else {//RESOLUTION
			        ...
			        face[0].btn(1,"BRAKING:",20,60,90,1365,0,0,66,239,132,euc.dash.ampL+ " A",35,182,84);
                    face[0].btn(1,"UPHILL:",20,60,23,1365,0,0,0,239,63,euc.dash.ampH+" A",35,180,16);
					face[0].ntfy("ONE PULSE PER "+euc.dash.ampS+ " A","",18,1365,1);
				}
			...
			}else  {buzzer(D16,1,40);face[0].set=0;face[0].init();}
			...
			//if (set.read("dash","slot"+set.read("dash","slot")+"Maker")!="Kingsong") 
				face[0].btn(1,"SPEED (in //"+((set.def.dash.mph)?"MPH)":"Km/h)"),18,120,8,1365,0,0,0,239,97,(set.def.dash.mph)?(euc.dash[euc.dash.haSv]*0.625).toFixed(1):euc.dash[euc.dash.haSv],50,120,40);
			//else 
			//	face[0].btn(1,"SPEED (in "+((set.def.dash.mph)?"MPH)":"Km/h)"),18,120,8,1365,0,0,0,239,97,(set.def.dash.mph)?(euc.dash.lim[euc.dash.haSv]*0.625).toFixed(1):euc.dash.lim[euc.dash.haSv],50,120,40);
			...
			if (x<=120&&y<100) { //Speed
				...
				face[0].ntfy("HOLD -> SET SPEED","HOLD -> SET SPEED",18,1365,1);
				buzzer(D16,1,[30,50,30]);
