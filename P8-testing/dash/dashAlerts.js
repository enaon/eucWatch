...
face[0] = {
	...
	init: function(){
		...
		this.btn(euc.dash.hapA,"AMPERE",24,185,20,col("blue1"),col("raf4"),122,0,239,97,(euc.dash.hapA)?"HAPTIC ON":"HAPTIC OFF",14,185,70);
		...
		if (face[0].set) { 
			...
			if (face[0].set=="spd") { 
				if (y<=120){ //spd
					...
					face[0].btn(1,"<      SPEED      >",20,120,20,col("dgray"),0,0,0,239,97,euc.dash[euc.dash.haSv]+" Km/h",25,120,60);
					...
					face[0].btn(1,"<       STEP       >",20,120,120,col("dgray"),0,0,100,239,195,euc.dash.spdS+ " km/h",25,120,160);
				...
				if (y<=65){ //uphill
					...
					face[0].btn(1," UPHILL:",20,80,20,col("dgray"),0,0,0,239,63,euc.dash.ampH+" A",28,200,20);
					face[0].ntfy("HAPTIC STARTS AT::","",18,col("dgray"),1);
				}else if (65<=y&&y<=133){//braking
					if (x<=120&&euc.dash.ampL<-5) euc.dash.ampL++;
					else if (120<=x&&-15<euc.dash.ampL) euc.dash.ampL--;
					face[0].btn(1,"BRAKING:",20,80,90,col("dgray"),0,0,66,239,132,euc.dash.ampL+ " A",28,200,90);
					face[0].ntfy("HAPTIC STARTS AT::","",18,col("dgray"),1);                    
				}else {//step
					if (120<=x&&euc.dash.ampS<3) euc.dash.ampS++;
					else if (x<=120&&1<euc.dash.ampS) euc.dash.ampS--;
					face[0].btn(1,"STEP:",20,80,160,col("dgray"),0,0,135,239,195,euc.dash.ampS+ " A",28,200,160);
					face[0].ntfy("ONE PULSE PER:","",18,col("dgray"),1);
				}
			...
			if (x<=120&&y<100) { //Speed
				...
				face[0].btn(euc.dash.hapS,"SPEED",24,60,20,col("blue1"),col("raf4"),0,0,119,97,(euc.dash.hapS)?"HAPTIC ON":"HAPTIC OFF",14,60,70);
				face[0].ntfy("HOLD -> SET SPEED","HOLD -> SET SPEED",18,col("dgray"),1);
				digitalPulse(D16,1,[30,50,30]);
			}else if (120<=x&&y<=100) { //Ampere
				euc.dash.hapA=1-euc.dash.hapA;
				face[0].btn(euc.dash.hapA,"AMPERE",24,185,20,col("blue1"),col("raf4"),122,0,239,97,(euc.dash.hapA)?"HAPTIC ON":"HAPTIC OFF",14,185,70);
				face[0].ntfy("HOLD -> SET AMPERE","",18,col("dgray"),1);
				digitalPulse(D16,1,[30,50,30]);
			}else if (x<=120&&100<=y) { //Temp
				euc.dash.hapT=1-euc.dash.hapT;
				face[0].btn(euc.dash.hapT,"TEMP",24,60,120,col("blue1"),col("raf4"),0,100,119,195,(euc.dash.hapT)?"HAPTIC ON":"HAPTIC OFF",14,60,170);
				face[0].ntfy("HOLD -> SET TEMP","",18,col("dgray"),1);
				digitalPulse(D16,1,[30,50,30]);		
			}else if (120<=x&&100<=y) { //Batt
				euc.dash.hapB=1-euc.dash.hapB;
				face[0].btn(euc.dash.hapB,"BATT",24,185,120,col("blue1"),col("raf4"),122,100,239,195,(euc.dash.hapB)?"HAPTIC ON":"HAPTIC OFF",14,185,170);
