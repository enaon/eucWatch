...
face[0] = {
	...
	init: function(){
       ...
       //if (!face.appPrev.startsWith("dash")) this.g.clear();
		...
		this.btn(euc.dash.hapA,"AMP",25,180,37,col("raf"),col("dgray"),122,0,239,97);
		this.btn(euc.dash.hapT,"TEMP",25,60,136,col("raf"),col("dgray"),0,100,119,195);
        this.btn(euc.dash.hapB,"BATT",25,180,136,col("raf"),col("dgray"),122,100,239,195);			
		this.ntfy("HOLD FOR HAPTIC","",22,col("black"),1);
        this.run=true;
	},
	show : function(){
		if (euc.state!=="READY"&&face.appPrev!=="dashGarage") {face.go(set.dash[set.def.dash.face],0);return;}
...
touchHandler[0]=function(e,x,y){ 
			...
			if (face[0].set=="spd") {
				...
				if (y<=65){ //uphill
					...
					if (!face[0].ampa) { face[0].ampa=1;face[0].ampd=0;face[0].ampr=0;
						face[0].btn(1,"BRAKING:",20,65,90,col("dgray"),0,0,66,239,132,euc.dash.ampL+ " A",35,182,84);
					...
					if (!face[0].ampd) { face[0].ampa=0;face[0].ampd=1;face[0].ampr=0;
						face[0].btn(1,"UPHILL:",20,65,23,col("gray"),0,0,0,239,63,euc.dash.ampH+" A",35,180,16);
						face[0].btn(1,"RESOLUTION:",17,70,157,col("gray"),0,0,135,239,195,euc.dash.ampS+ " A",35,190,150);
					}
   					return setTimeout(function() {
	...
	case 12: //hold event
		if (face[0].set) { 
			...
			face[0].btn(euc.dash.hapS,"SPEED",25,65,37,col("raf"),col("dgray"),0,0,119,97);
