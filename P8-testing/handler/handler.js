	...
	task:{
		euc:{
			hour:x=>{
				...
				require('Storage').list(/logDaySlot/).forEach(logfile=>{set.write(logfile,x,0);});
				euc.log.trp[0]=0;
			},
			day:x=>{
				let pr=(!x)?6:x-1;
				...
				require('Storage').list(/logWeekSlot/).forEach(logfile=>{set.write(logfile,x,0);});
				euc.log.trp[1]=0;
			},
			month:x=>{
				let pr=(!x)?11:x-1;
				...
				require('Storage').list(/logYearSlot/).forEach(logfile=>{set.write(logfile,x,0);});
				euc.log.trp[2]=0;
			}
		}
	}
