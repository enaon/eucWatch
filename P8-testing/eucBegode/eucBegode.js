	...
	//read
		...
		c.on('characteristicvaluechanged', function(event) {
			if  ( event.target.value.buffer[0]===85 ) {
				...
				euc.dash.trpT=((((event.target.value.buffer[6]&0xFF) <<24) + ((event.target.value.buffer[7]&0xFF) << 16) + ((event.target.value.buffer[8] & 0xFF) <<8) + (event.target.value.buffer[9] & 0xFF))/1000).toFixed(1);
