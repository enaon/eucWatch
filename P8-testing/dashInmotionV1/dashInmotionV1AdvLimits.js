	...
	case 5://tap event
        if (!face[0].setE){//select page
			if (x<=120&&y<100) { //alarm 1
				...
				if ( euc.dash.limE[1]){
					if (euc.dash.limE[0]) {face[0].ntfy("DISABLE ALARM 1","",20,col("red"),1);buzzer(40);}
					else {
						euc.dash.limE[1]=0;
						buzzer([30,50,30]);
			...
			if (120<=x) { //up
                ...
                if (face[0].setEb=="0") {
                    ...
                    if (euc.dash.lim[0]==0)  {euc.dash.lim[0]=1;face[0].ntfy("DISABLED","",20,col("red"),1);euc.dash.limE[0]=0;buzzer(40);return;}
                }else if (face[0].setEb=="1"){
					if (euc.dash.limE[0]){
						if (euc.dash.lim[1]<=5) { face[0].ntfy("AL1 DN LIMIT IS 5KpH","",20,col("red"),1);buzzer(40);return;}
