...
//		this.g.drawString("MANUALY SET PEDAL TILT",120-(this.g.stringWidth("MANUALY SET PEDAL TILT")/2),5);
		...
		this.g.drawString(euc.dash.set.pTlt,130-(this.g.stringWidth(euc.dash.set.pTlt)/2),65); 		
		this.g.flip(); 
	...
	show : function(){
        ...
        if (this.calibrate) {
		    ...
		    this.g.flip();
       		this.run=false;
        }else if (this.tilt!=euc.dash.set.pTlt){ //tilt Set
		    this.g.setColor(0,1);
...
touchHandler[0]=function(e,x,y){ 
	...
	case 5: //tap event
		...
		if (x<=120&&y<175) { //tilt forward
			euc.dash.set.pTlt--;euc.wri("setCalibrateTilt",euc.dash.set.pTlt);
			buzzer([30,50,30]);
